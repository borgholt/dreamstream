...
import random
from dreamstream.utils import test as t
...
from dreamstream.func_coverage import SUPPORTED_FUNCTIONS, UNSUPPORTED_FUNCTIONS, SUPPORTED_NON_OVERRIDEABLE_FUNCTIONS
from dreamstream import StreamTensor
from dreamstream.utils import test as t

FLAT = {f for k, fs in torch.overrides.get_overridable_functions().items() for f in fs}
...
for kernel_size in range(1, 3):
    for stride in range(1, 3):
        ...
        conv = nn.Conv1d(256, 128, kernel_size=kernel_size, stride=stride, padding=kernel_size//2)
        ...
        conv.online()
        data = {_id: conv(s) for _id, s in zip(ids, data)}   
        # TEMP TEST

        for _id, s in data.items():
            assert s.size("L") == s.meta.lengths[0]
            assert torch.allclose(s, targets[_id])
            
        print(f"OK: {stride > kernel_size} {kernel_size}, {stride}")
