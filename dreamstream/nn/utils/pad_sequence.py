...
def pad_stream_tensor(sequences, batch_first: bool = False) -> StreamTensor:
    meta = StreamMetadata.cat_batch([t.meta for t in sequences])
    names = sequences[0].names
    sequences = [t.tensor() for t in sequences]

    return pad_chunks(
        sequences=sequences, names=names, ids=meta.ids, sos=meta.sos, eos=meta.eos, batch_first=batch_first
    )
