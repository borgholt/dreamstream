...
class TestRNNs:
    ...
    test_modules = [
        nn.RNN(16, 16, bidirectional=False, batch_first=True),
    ]
    ...
    @pytest.mark.parametrize("module", test_modules)
    def test_equivalence(self, sequences, ids, batches_of_chunks, module, initial_state):
        if initial_state:
        ...
        with torch.inference_mode():
            targets = {_id: module(s.transpose(0, 1).unsqueeze(0), state) for _id, s in zip(ids, sequences)}
