...
OVERRIDES = {
    ...
    # torch.jit.script(torch.nn.functional.linear): linear_override,
}


def mm_override(input, other):
    output = aten.mm(input.tensor, other)
    return as_metadata_tensor(output, input.metadata)


def convolution_override(
    input,
    weight,
    bias=None,
    stride=1,
...
ATEN_OVERRIDES = {
    # aten.mm: mm_override,
    # aten.matmul: mm_override,
    # aten.addmm: addmm_flop_jit,
    # aten.bmm: bmm_flop_jit,
    aten.convolution: convolution_override,
}
...
class MetadataTensor(torch.Tensor):
    ...
    def __torch_dispatch__(cls, func, types, args=(), kwargs=None):
        ...
        if func in ATEN_OVERRIDES:
            print(f"Calling aten override for {func}")
            out = ATEN_OVERRIDES[func](*args, **kwargs, metadata=metas[0], state=states[0])
        else:
